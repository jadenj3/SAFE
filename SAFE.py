...
class Learner(BaseLearner):
            ...
            for class_index in np.unique(self.train_dataset.labels):
                ...
                if self.is_dil:
                    ...
                    self._network.fc.weight.data[class_index] += class_prototype.to(
                        device='cuda')  # for dil, we update all classes in all tasks
                else:
    ...
    def incremental_train(self, data_manager):
        ...
        if self._cur_task > 0 and self.args['use_RP'] and self.args['M'] > 0:
            ...
            self._network.update_fc(
                self._classes_seen_so_far)  # creates a new head with a new number of classes (if CIL)
        ...
        self.train_dataset = data_manager.get_dataset(np.arange(self._known_classes, self._classes_seen_so_far),
                                                      source="train", mode="train", )  # mode
        self.train_loader = DataLoader(self.train_dataset, batch_size=int(self._batch_size), shuffle=True,
                                       num_workers=num_workers)
        train_dataset_for_CPs = data_manager.get_dataset(np.arange(self._known_classes, self._classes_seen_so_far),
                                                         source="train", mode="test", )
        self.train_loader_for_CPs = DataLoader(train_dataset_for_CPs, batch_size=self._batch_size, shuffle=True,
                                               num_workers=num_workers)  # 求协方差矩阵和每个类的性质
    ...
    def _train(self, train_loader, test_loader, train_loader_for_CPs):
        ...
        if self.args["model_name"] in ['joint_linear', 'joint_full']:
            ...
            optimizer = optim.SGD([{'params': self._network.convnet.parameters()},
            ...
            if self._cur_task == 0 and self.dil_init == False:
                ...
                if self.args["model_name"] != 'ncm':  # this is called by default
                    self.show_num_params()
                    optimizer = optim.SGD([{'params': self._network.parameters()}], momentum=0.9,
                                          lr=self.args['body_lr'], weight_decay=self.weight_decay)
                ...
                if self.args['follow_epoch']:
                    ...
                    if self.args["model_name"] != 'ncm':  # this is called by default
                        ...
                        optimizer = optim.SGD(self._network.parameters(), momentum=0.9, lr=self.args['body_lr'],
                                              weight_decay=self.weight_decay)
                        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.args['tuned_epoch'],
                                                                         eta_min=self.min_lr)
                        logging.info(
                            "Starting PETL training on first task using " + self.args["model_name"] + " method")
    ...
    def setup_RP_branch(self):
        ...
        if self.args['M'] > 0:
            ...
            self.model_branch1.fc.weight = nn.Parameter(
                torch.Tensor(self.model_branch1.fc.out_features, M).to(device='cuda'))  # num classes in task x M
            ...
            self.W_rand_branch = copy.deepcopy(
                self.model_branch1.fc.W_rand)  # make a copy that gets passed each time the head is replaced
        else:
    ...
    def setup_RP(self):
        ...
        if self.args['M'] > 0:
            ...
            self._network.fc.weight = nn.Parameter(
                torch.Tensor(self._network.fc.out_features, M).to(device='cuda'))  # num classes in task x M
            ...
            self.W_rand = copy.deepcopy(
                self._network.fc.W_rand)  # make a copy that gets passed each time the head is replaced
        else:
            # no RP, only decorrelation
